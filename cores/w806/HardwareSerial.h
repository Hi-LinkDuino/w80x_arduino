...
class HardwareSerial : public Stream {
  ...
  public:
    HardwareSerial(uint8_t _rx, uint8_t _tx, uint8_t _rts = 0xFF, uint8_t _cts = 0xFF);         //默认不设置RTS CTS 引脚
    ...
    void setRx(uint8_t _rx);
    void setTx(uint8_t _tx);

    // Enable HW flow control on RTS, CTS or both
    void setRts(uint8_t _rts);
    void setCts(uint8_t _cts);
    void setRtsCts(uint8_t _rts, uint8_t _cts);
  ...
  private:
    ...
    uint8_t _config;
    unsigned long _baud;
    void init(uint8_t _rx, uint8_t _tx, uint8_t _rts = 0xff, uint8_t _cts = 0xff);
